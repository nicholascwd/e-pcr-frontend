...
function App() {
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/networkTest`,{timeout:3000}).then((response) => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/users/verifyToken?token=${token}`,{timeout:4000}).then((response) => {
  ...
  return (
    ...
    >
      ...
      <Menu theme="dark" mode="inline" defaultSelectedKeys={['1']}>
            <Menu.Item key="1" icon={<FileTextOutlined />} onClick={() => history.push('/select_patient')}>
            Forms
            </Menu.Item>
            <Menu.Item key="2" icon={<SettingOutlined />} onClick={()=> history.push('/residents_module')}>
              Residents Module
            </Menu.Item>
            <Menu.Item key="3" icon={<BellOutlined />}  onClick={() => history.push('/announcement')}>
              Announcements
            </Menu.Item>
            <Menu.Item key="4" icon={<BellOutlined />}  onClick={() => history.push('/settings')}>
              Settings
            </Menu.Item>
      </Menu>
    </Sider>
            ...
            <Content>
              <div className="content">
                <Switch>
                  ...
                  <PrivateRoute exact path={['/select_patient', '/']} component={SelectPatient} />
                  <PrivateRoute exact path={['/patient_profile/:bedNo', '/']} component={PatientProfile} />
                  <PrivateRoute exact path={['/forms/restraints_form/:uuid', '/']} component={RestraintsForm} />
                  <PrivateRoute exact path={['/forms/progress_record_form/:uuid', '/']} component={ProgressRecordForm} />
                  <PrivateRoute exact path={['/residents_module/', '/']} component={ResidentsModuleMainList} />
                  <PrivateRoute exact path={['/residents_module/admit', '/']} component={AdmitResident} />
