...
export const restraintsPdfExport =((patientData, restraintsSubmissions)=>{
    ...
    for (let i=0; i<restraintsSubmissions.length;i++){
      ...
      row.push(JSON.parse(restraintsSubmissions[i].formVals).['r-restraints-type'])
    ...
    doc.autoTable({
      startY: 50,
      head: [['Date', 'Time', 'Applied Correctly','Comfortable','Circulation','ROM','Skin Checked','Restraints Type','Remarks','Staff Signature']],
      body: tableData,
    })
